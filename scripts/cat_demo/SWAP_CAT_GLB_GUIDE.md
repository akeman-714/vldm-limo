...
- 远端 = `train-server`（免密 ssh），仓库 `/home/asong/vlfm`，场景目录
...
ssh train-server 'mkdir -p /home/asong/vlfm/data/scene_datasets/hm3d/val/00800-TEEsavR23oF/_incoming'
scp clean_cowcat.glb twocat_layout.glb \
  train-server:/home/asong/vlfm/data/scene_datasets/hm3d/val/00800-TEEsavR23oF/_incoming/
...
```bash
ssh train-server
cd /home/asong/vlfm && conda activate vlfm_pip
...
```bash
# 传上去(注意去 CRLF)再跑
scp scripts\cat_demo\_run_cowcat.sh train-server:/home/asong/vlfm/scripts/cat_demo/_run_cowcat.sh
