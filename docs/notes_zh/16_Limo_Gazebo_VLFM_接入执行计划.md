...
> 状态提示（2026-06-30）：这篇是接入前的执行计划，不是最终实现说明。Limo Gazebo + VLFM + Nav2 的 G2/G3 耦合已经在线验收通过；最终架构、接口、运行命令和验收结果见 [26_Limo_Gazebo_VLFM_最终耦合说明与接口.md](./26_Limo_Gazebo_VLFM_最终耦合说明与接口.md)。
